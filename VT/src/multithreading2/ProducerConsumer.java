...
public class ProducerConsumer {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for (int i = 0; i < numConsumers - remainder; i++) {
			consumers[i] = new Consumer(i + 1, synchProducts, average);
		}
		for (int i = numConsumers - remainder; i < numConsumers; i++) {
			consumers[i] = new Consumer(i + 1, synchProducts, average + 1);
		}
		//create producers
		for (int i = 0; i < numProducers; i++) {
			producers[i] = new Producer(i + 1, synchProducts, toProduce);
		}
